...
from .inference import INPUT_NODE, OUTPUT_NODE, IMAGE_SIZE, NUM_CHANNELS, inference
...
def train(mnist):
    ...
    with tf.name_scope('output'):
        ## 预测输出
        Ylogits = inference(X, regularizer, train=True)
    ...
    with tf.name_scope('train'):
        ...
        # FIXME WHY 使用梯度下降优化，参数收敛特别慢
        train_step = tf.train.AdadeltaOptimizer(decayed_learning_rate) \
